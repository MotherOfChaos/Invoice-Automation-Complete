/**
 * ═══════════════════════════════════════════════════════════════════════════
 * INVOICE AUTOMATION SYSTEM v1.2
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * For: Teatro Metamorfosis
 * Created: 2025-01-19
 * Updated: 2025-01-19 v1.2 - FIXED: Search query + Vision API extraction
 *
 * WHAT THIS DOES:
 ...
 * CRITICAL: Search for ALL attachments, then filter by filename (not by email subject)
 */
function buildSearchQuery(onlyNew = false) {
  const excludeSent = CONFIG.emailAccounts.map(email => `-from:${email}`).join(' ');

  // IMPORTANT: Search for ALL emails with attachments (not keywords in subject)
  // We filter by filename later - this finds invoices regardless of email subject
