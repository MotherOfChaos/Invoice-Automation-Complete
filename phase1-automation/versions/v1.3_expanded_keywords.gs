/**
 * ═══════════════════════════════════════════════════════════════════════════
 * INVOICE AUTOMATION SYSTEM v1.3
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * For: Teatro Metamorfosis
 * Created: 2025-01-19
 * Updated: 2025-01-19 v1.3 - EXPANDED KEYWORDS: Search email subjects + filenames
 *
 * WHAT THIS DOES:
...
function scanAndExtractEmails(folderStructure, onlyNew = false) {
  ...
  threads.forEach(thread => {
    ...
    messages.forEach(message => {
      ...
      // Get email subject for keyword matching
      const subject = message.getSubject().toLowerCase();

      // Check if email subject OR any attachment filename contains invoice keywords
      const hasInvoiceKeyword = CONFIG.invoiceKeywords.some(keyword =>
        subject.includes(keyword.toLowerCase())
      );

      // If email subject doesn't have keywords, check attachments individually
      ...
      attachments.forEach(attachment => {
        try {
          ...
          // FILE TYPE FILTER: Check if email subject OR filename contains invoice keywords
          // CASE-INSENSITIVE: All keywords converted to lowercase for comparison
          const filenameHasKeyword = CONFIG.invoiceKeywords.some(keyword =>
            filename.includes(keyword.toLowerCase())
          );

          if (!hasInvoiceKeyword && !filenameHasKeyword) {
          ...
          const newFileName = `${datePrefix}_${originalName}`;
          ...
          Logger.log(`  ✓ Organized: ${newFileName} → ${year}/${CONFIG.quarters[quarter - 1]}`);
...
function buildSearchQuery(onlyNew = false) {
  ...
  // IMPORTANT: Search for ALL emails with attachments
  // We filter by email subject + filename keywords in scanAndExtractEmails()
  let query = `has:attachment ${excludeSent} in:anywhere`;
