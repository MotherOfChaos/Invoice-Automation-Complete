/**
 * ═══════════════════════════════════════════════════════════════════════════
 * INVOICE AUTOMATION SYSTEM v1.6
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * For: Teatro Metamorfosis
 * Created: 2025-01-19
 * Updated: 2025-01-19 v1.6 - SMART FILTERING: Filename keywords + skip-words + control folder
 *
 * WHAT THIS DOES:
...
const CONFIG = {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // ===== SKIP-WORDS (Filenames to reject even if small) =====
  // ═══════════════════════════════════════════════════════════════════════════
  //
  // These words in attachment filenames indicate non-invoice files like
  // icons, logos, headers, badges that should ALWAYS be skipped.
  //
  ...
  // ===== CONTROL FOLDER (for analyzing skipped files) =====
...
function scanAndExtractEmails(folderStructure, onlyNew = false) {
  ...
  threads.forEach(thread => {
    ...
    messages.forEach(message => {
      ...
      // Get or create control folder for skipped files
      let controlFolder = null;
      if (CONFIG.enableControlFolder) {
        const controlFolders = DriveApp.getFoldersByName(CONFIG.controlFolderName);
        if (controlFolders.hasNext()) {
          controlFolder = controlFolders.next();
        } else {
          controlFolder = DriveApp.createFolder(CONFIG.controlFolderName);
          Logger.log(`📁 Created control folder: ${CONFIG.controlFolderName}`);
        }
      }

      // Process attachments with SMART filtering
      attachments.forEach(attachment => {
        try {
          ...
          // ═══════════════════════════════════════════════════════════════════
          // SMART FILTERING LOGIC (v1.6)
          // ═══════════════════════════════════════════════════════════════════
          //
          // STEP 1: Check if filename contains INVOICE keywords
          //         → If YES: SAVE IT (bypass size check)
          //
          // STEP 2: Check if filename contains SKIP-WORDS
          //         → If YES: SKIP IT (definitely not an invoice)
          //
          // STEP 3: Check file size
          //         → If < 50KB: SKIP IT (likely icon/logo)
          //         → If >= 50KB: SAVE IT
          //
          // STEP 4: Check content type
          //         → Skip GIF, ICO, HTML, plain text
          //
          // ═══════════════════════════════════════════════════════════════════
          ...
          // STEP 1: Check if filename contains invoice keywords
          const filenameHasKeyword = CONFIG.invoiceKeywords.some(keyword =>
            fileNameLower.includes(keyword.toLowerCase())
          );

          if (filenameHasKeyword) {
            // Filename has invoice keyword - SAVE IT regardless of size
            Logger.log(`  ✓ Filename has keyword, saving regardless of size: ${fileName} (${Math.round(fileSize/1024)}KB)`);
            // Continue to save (skip all other checks)
          } else {
            // STEP 2: Check if filename contains skip-words
            const filenameHasSkipWord = CONFIG.skipWords.some(word =>
              fileNameLower.includes(word.toLowerCase())
            );

            if (filenameHasSkipWord) {
              skipReason = `skip-word in filename`;
            }
            // STEP 3: Check file size (only if no invoice keyword)
            else if (fileSize < 50000) { // 50KB threshold
            ...
            // STEP 4: Check content type
              ...
              if (skipTypes.some(type => contentType.includes(type))) {
                skipReason = `non-document type (${contentType})`;
              }
            }
          }

          // If we have a skip reason, save to control folder and skip
          if (skipReason) {
            Logger.log(`  ⊘ Skipped (${skipReason}): ${fileName}`);

            // Save to control folder if enabled
            if (CONFIG.enableControlFolder && controlFolder) {
              try {
                const datePrefix = Utilities.formatDate(messageDate, Session.getScriptTimeZone(), 'yyyy-MM-dd');
                const controlFileName = `${datePrefix}_[SKIPPED]_${fileName}`;
                const blob = attachment.copyBlob();
                controlFolder.createFile(blob).setName(controlFileName);
                Logger.log(`    💾 Saved to control folder: ${controlFileName}`);
              } catch (e) {
                Logger.log(`    ⚠️ Could not save to control folder: ${e.toString()}`);
              }
            }
