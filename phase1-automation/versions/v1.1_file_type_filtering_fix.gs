/**
 * ═══════════════════════════════════════════════════════════════════════════
 * INVOICE AUTOMATION SYSTEM v1.1
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * For: Teatro Metamorfosis
 * Created: 2025-01-19
 * Updated: 2025-01-19 - FIXED: File type filtering (no more icons/images)
 *
 * WHAT THIS DOES:
...
function buildSearchQuery(onlyNew = false) {
  const keywords = CONFIG.invoiceKeywords.map(kw => `"${kw}"`).join(' OR ');
  const excludeSent = CONFIG.emailAccounts.map(email => `-from:${email}`).join(' ');

  let query = `(${keywords}) ${excludeSent} has:attachment in:anywhere`;
